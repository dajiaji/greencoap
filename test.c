...
void test_gcoap_sample_readme() {
  ...
  assert(gcoap_serializer_add_opt(s, O_URI_HOST, "example.com",
                                  sizeof("example.com") - 1) == GCOAP_OK);
  assert(gcoap_serializer_add_opt_uint(s, O_URI_PORT, 5683) == GCOAP_OK);
  assert(gcoap_serializer_add_opt(s, O_URI_PATH, "/temperature",
                                  sizeof("/temperature") - 1) == GCOAP_OK);
